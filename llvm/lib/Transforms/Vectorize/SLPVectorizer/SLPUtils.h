...
/// register domain, so the extracts are paid on top of the repacking. An
/// extract is a seed only if it builds a vector, such a pair is folded into a
/// shuffle, while the other extracts just tear the vector apart.
