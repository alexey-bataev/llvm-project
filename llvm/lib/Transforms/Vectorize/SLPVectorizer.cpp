...
private:
  ...
  public:
    ...
    ArrayRef<Instruction *> getMemoryDependencies() const {
      return MemoryDependencies;
    }
    /// Adds a memory dependency.
    void addMemoryDependency(Instruction *Inst) {
      MemoryDependencies.push_back(Inst);
    }
    /// Gets the list of control dependencies.
    ArrayRef<Instruction *> getControlDependencies() const {
      return ControlDependencies;
    }
    /// Adds a control dependency.
    void addControlDependency(Instruction *Inst) {
      ControlDependencies.push_back(Inst);
    }

    /// Gets/sets the next load/store instruction in the block.
    Instruction *getNextLoadStore() const { return NextLoadStore; }
    void setNextLoadStore(Instruction *Next) { NextLoadStore = Next; }
  ...
  private:
    ...
    Instruction *NextLoadStore = nullptr;

    /// The dependent memory instructions.
    /// This list is derived on demand in calculateDependencies().
    SmallVector<Instruction *> MemoryDependencies;
    ...
    SmallVector<Instruction *> ControlDependencies;
  ...
  public:
    ...
    bool isScheduled() const {
      return all_of(Bundle, [](ScheduleData *SD) { return SD->isScheduled(); });
    }
    ...
    bool hasValidDependencies() const {
      return all_of(
          Bundle, [](ScheduleData *SD) { return SD->hasValidDependencies(); });
    }
    ...
    void dump(raw_ostream &OS) const {
      ...
      OS << '[';
      interleaveComma(Bundle, OS, [&](const ScheduleData *SD) { OS << *SD; });
      OS << ']';
    }
  ...
  struct BlockScheduling {
    ...
    void schedule(ScheduleEntity *Data, ReadyListType &ReadyList) {
      auto ProcessBundleMember = [&](ScheduleData *BundleMember,
        ...
        auto DecrUnsched = [&](Instruction *I, ScheduleData *Data,
                               bool IsControl) {
        ...
        auto DecrUnschedForInst = [&](Instruction *I, bool IsControl = false) {
          if (ScheduleData *OpSD = getScheduleData(I))
            DecrUnsched(I, OpSD, IsControl);
        };
        ...
        for (Instruction *MemoryDep : BundleMember->getMemoryDependencies()) {
          // There are no more unscheduled dependencies after decrementing,
          // so we can put the dependent instruction into the ready list.
          LLVM_DEBUG(dbgs()
                     << "SLP:   check for readiness (mem): " << *MemoryDep
                     << "\n");
          DecrUnschedForInst(MemoryDep);
        }
        // Handle the control dependencies.
        for (Instruction *Dep : BundleMember->getControlDependencies()) {
          // There are no more unscheduled dependencies after decrementing,
          // so we can put the dependent instruction into the ready list.
          LLVM_DEBUG(dbgs()
                     << "SLP:   check for readiness (ctrl): " << *Dep << "\n");
          DecrUnschedForInst(Dep, /*IsControl=*/true);
        }
      };
...
BoUpSLP::BlockScheduling::tryScheduleBundle(ArrayRef<Value *> VL, BoUpSLP *SLP,
  ...
  for (Value *V : VL) {
    ...
    if (!extendSchedulingRegion(V, S)) {
      ...
      // scheduling.
      ScheduleBundle Empty;
      TryScheduleBundleImpl(/*ReSchedule=*/false, Empty);
      return std::nullopt;
    }
  }
...
void BoUpSLP::BlockScheduling::initScheduleData(Instruction *FromI,
  ...
  for (Instruction *I = FromI; I != ToI; I = I->getNextNode()) {
    ...
    if (I->mayReadOrWriteMemory() &&
      ...
      if (CurrentLoadStore) {
        CurrentLoadStore->setNextLoadStore(I);
      } else {
  ...
  if (NextLoadStore) {
    if (CurrentLoadStore)
      CurrentLoadStore->setNextLoadStore(NextLoadStore->getInst());
  } else {
...
void BoUpSLP::BlockScheduling::calculateDependencies(ScheduleBundle &Bundle,
  ...
  auto ProcessNode = [&](ScheduleData *BundleMember) {
    ...
    auto MakeControlDependent = [&](Instruction *I) {
      ...
      DepDest->addControlDependency(BundleMember->getInst());
      BundleMember->incDependencies();
    ...
    Instruction *LSInst = BundleMember->getNextLoadStore();
    if (!LSInst)
      return;
    ...
    for (ScheduleData *DepDest = getScheduleData(LSInst); DepDest;
         DepDest = DepDest->getNextLoadStore()
                       ? getScheduleData(DepDest->getNextLoadStore())
                       : nullptr) {
      ...
      if (DistToSrc >= MaxMemDepDistance ||
        ...
        DepDest->addMemoryDependency(SrcInst);
        BundleMember->incDependencies();
        if (!DepDest->isScheduled())
          BundleMember->incrementUnscheduledDeps(1);
        WorkList.push_back(DepDest);
  ...
  while (!WorkList.empty()) {
    ...
    if (!Bundles.empty()) {
      for (ScheduleBundle *Bundle : Bundles) {
        if (!Visited.insert(Bundle).second || Bundle->isScheduled())
          continue;
        assert(isInSchedulingRegion(*Bundle) &&
