...
void BoUpSLP::tryToVectorizeGatheredLoads(
  ...
  if (!GatheredLoads.empty() && !NonVectorized.empty() &&
    ...
    if (!FinalGatheredLoads.empty() && !FinalGatheredLoads.back().empty()) {
      LoadInst *LI = FinalGatheredLoads.back().back().first;
      SmallVector<Value *> VL(FinalGatheredLoads.back().size());
      for (auto [V, P] : zip(VL, FinalGatheredLoads.back()))
        V = P.first;
      Align Alignment = computeCommonAlignment<LoadInst>(VL);
      auto *Ty = getWidenedType(LI->getType(), VL.size());
      if (TTI->isLegalMaskedGather(Ty, Alignment) &&
  ...
  for (std::unique_ptr<TreeEntry> &TE : VectorizableTree) {
    ...
    if (E.isGather() && !isSplat(E.Scalars) &&
        (E.getOpcode() == Instruction::Load ||
         (!E.getOpcode() && any_of(E.Scalars, IsaPred<LoadInst>))))
