...
bool BoUpSLP::isProfitableToReorder() const {
  if (VectorizableTree.front()->hasState() &&
    ...
    // Check if the tree has only single store and single load node, other nodes
    // are phis.
    unsigned Loads = 0;
    unsigned Phis = 0;
    constexpr unsigned TinyVF = 2;
    const TreeEntry *BinOpTE = nullptr;
    unsigned BinOps = 0;
    for (const std::unique_ptr<TreeEntry> &TE :
      ...
      if (!TE->hasState()) {
        if (all_of(TE->Scalars, IsaPred<Constant, PHINode>))
          continue;
        if (TE->Scalars.size() == TinyVF &&
            any_of(TE->Scalars, IsaPred<PHINode>))
          continue;
        return true;
      }
      ...
      if (TE->State == TreeEntry::Vectorize && TE->Scalars.size() == TinyVF &&
          Instruction::isBinaryOp(TE->getOpcode())) {
        ++BinOps;
        if (BinOpTE) {
          if (BinOps <= 2 &&TE->getVectorFactor() == TinyVF &&
              BinOpTE->getOpcode() == TE->getOpcode())
            continue;
          return true;
        }
        BinOpTE = TE.get();
    ...
    return Phis <= VectorizableTree.size() / 4;
  }
  return true;
}
