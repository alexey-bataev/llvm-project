...
void BoUpSLP::tryToVectorizeGatheredLoads(
  ...
  auto GetVectorizedRanges = [this](ArrayRef<LoadInst *> Loads,
    ...
    if (VectorizeNonPowerOf2 && has_single_bit(Loads.size() + 1))
          ...
          auto CheckIfAllowed = [=](ArrayRef<LoadInst *> Slice) {
            for (LoadInst *LI : Slice) {
              ...
              if (LI->hasNUsesOrMore(UsesLimit) ||
                  std::distance(LI->user_begin(), LI->user_end()) !=
                      LI->getNumUses())
                return false;
              for (User *U : LI->users()) {
  ...
  if (!GatheredLoads.empty() && !NonVectorized.empty() &&
    ...
    ArrayRef<Value *> Values(
        reinterpret_cast<Value *const *>(NonVectorized.begin()),
        NonVectorized.size());
    Align Alignment = computeCommonAlignment<LoadInst>(Values);
    auto *Ty =
        getWidenedType(NonVectorized.front()->getType(), NonVectorized.size());
...
void BoUpSLP::buildTree_rec(ArrayRef<Value *> VL, unsigned Depth,
  ...
  if (S.MainOp && !DT->isReachableFromEntry(S.MainOp->getParent())) {
